...
if [ -n "$dock" -a -d "$dock" ]; then
        ...
        notify-send -u critical -t 10000 "ThinkPad Ultrabay eject failed" "Cannot locate bay dock device"
fi
...
unmount_rdev() { perl - "$@" <<'EOPERL'  # let's do it in Perl
        ...
        foreach $m (@mounts) {
                ...
                if ($bad == 1) {
                        ...
                        system("notify-send -u critical -t 10000 \"Error unmounting $dir\" \"Unmounting of $dir on $dev failed!\"");
                } else {
...
if [ $( cat $dock/docked ) == 0 ]; then
    ...
    if [ -d $ULTRABAY_SYSDIR ]; then
        ...
        || {
            ...
            notify-send -u critical -t 10000 "ThinkPad Ultrabay eject failed" "Please do not pull the device, doing so could cause file corruption and possibly hang the system. Unmounting of the filesystem on the ThinkPad Ultrabay device failed. Please put the eject leaver back in place, and try to unmount the filesystem manually. If this succeeds you can try the eject again"
            exit 1;
        }
        sync
        ...
        notify-send -u critical -t 10000 "ThinkPad Ultrabay eject failed" "Bay occupied but incorrect device path"
        #echo 2 > /proc/acpi/ibm/beep  # triple error tone
        exit 1
    fi
fi
